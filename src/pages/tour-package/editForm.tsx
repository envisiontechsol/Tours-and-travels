...
} from "../../services/api/tours/toursApi";
import {
  closeAllEditAction,
  useEditMgmtStore,
...
const EditTourPackageForm: React.FC = () => {
  const editData = useEditMgmtStore((s) => s.editTourPackageData);
  ...
  const onSubmit = async (data: TourPackageFormValues) => {
    ...
    try {
      ...
      await updateTourPackageReq(editData?.id || "", formData);
      onCancelOrClose();
  ...
  useEffect(() => {
    if (editData) {
      ...
      reset({
        ...
        bannerAlt: editData?.bannerImagetage || "",
        bannerImage: undefined,
        code: editData?.code,
        fakePriceInINR: Number(editData?.fakePriceInINR || 0),
        flightMarginPct: Number(editData?.FlightMarginPct || 0),
        hotelMarginPct: Number(editData?.HotelMarginPct || 0),
        image1Alt: editData?.image1tage || "",
        image2Alt: editData?.image2tage || "",
        image3Alt: editData?.image3tage || "",
        image1Url: undefined,
        image2Url: undefined,
        image3Url: undefined,
        ...
        packageTypeId: selectedPackageType
          ? {
              label: selectedPackageType.label,
              value: String(selectedPackageType.value),
            }
          : undefined,
    ...
    <div className="mt-2 border rounded-lg p-5 shadow-sm bg-white relative">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-6">
        ...
        <div className="mt-6 flex items-center space-x-3">
          ...
          >
            {isSubmitting ? "Updating..." : "Update Tour Package"}
          </button>
