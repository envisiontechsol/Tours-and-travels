...
import { fetchQuickLinksReq } from "../../services/api/quickLinks/quickLinkApi";
import { useTableRefreshStore } from "../../store/tableRefreshStore";
import { QuickLinkResType } from "../../types/quickLinksTypes";
import { quickLinkColumns } from "./column";

const TableList = () => {
  const { refreshKey } = useTableRefreshStore();

  const [data, setData] = useState<QuickLinkResType[]>([]);
  ...
  const fetchData = useCallback(
    async (page: number, pageSize: number) => {
      ...
      try {
        const res = await fetchQuickLinksReq(page, pageSize);
