...
import blogColumns from "./column";
import { fetchBlogReq } from "../../services/api/blogs/blogsApi";

const TableList = () => {
  const { refreshKey } = useTableRefreshStore();

  const [data, setData] = useState<BlogPostResType[]>([]);
  ...
  const fetchData = useCallback(
    async (page: number, pageSize: number) => {
      ...
      try {
        const res = await fetchBlogReq(page, pageSize);
